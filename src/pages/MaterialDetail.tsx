...
const MaterialDetail: React.FC = () => {
    ...
    const { data: material, isLoading, isError } = useQuery<Material>({
    ...
    const updateStockQuantityMutation = useMutation({
        mutationFn: (stockQuantity: number) => materialsApi.updateStockQuantity(materialId || '', stockQuantity),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['material', materialId] })
    ...
    const updateStatusMutation = useMutation({
        mutationFn: (status: "ACTIVE" | "INACTIVE") => materialsApi.updateStatus(materialId || '', status),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['material', materialId] })
    ...
    if (isError || !material) {
        return (
            <div className="flex items-center justify-center h-screen">
                <div className="text-red-500">Failed to load material details</div>
            </div>
        )
    }
        ...
        <div className="w-full mt-[60px]">
            <div className="max-w-4xl mx-auto p-6">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Material Details</h1>
