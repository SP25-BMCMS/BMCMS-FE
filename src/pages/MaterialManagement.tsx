...
const MaterialManagement: React.FC = () => {
    ...
    const { data: materialsData, isLoading, isFetching } = useQuery<MaterialResponse>({
    ...
    const updateUnitPriceMutation = useMutation({
        mutationFn: (unitPrice: number) => materialsApi.updateUnitPrice(selectedMaterial?.material_id || "", unitPrice.toString()),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['materials'] })
    ...
    const handleUpdateStockQuantity = (stockQuantity: number) => {
        if (selectedMaterial) {
            updateStockQuantityMutation.mutate({ materialId: selectedMaterial.material_id, stockQuantity })
        }
    }
