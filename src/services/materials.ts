...
export const materialsApi = {
    ...
    updateUnitPrice: async (materialId: string, unitPrice: string): Promise<Material> => {
        const response = await apiInstance.put<Material>(`/materials/${materialId}/unit-price`, { unit_price: unitPrice })
        return response.data
    },

    updateStockQuantity: async (materialId: string, stockQuantity: number): Promise<Material> => {
        const response = await apiInstance.put<Material>(`/materials/${materialId}/stock-quantity`, { stock_quantity: stockQuantity })
        return response.data
    },
