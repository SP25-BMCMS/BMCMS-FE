...
const scheduleJobsApi = {
    ...
    getScheduleJobs: async (params: { page: number; limit: number; schedule_id?: string }): Promise<ScheduleJobListResponse> => {
    ...
    fetchScheduleJobsByScheduleId: async (scheduleId: string, params?: { page: number; limit: number }): Promise<ScheduleJobListResponse> => {
    ...
    updateScheduleJob: async (scheduleJobId: string, data: UpdateScheduleJobRequest): Promise<ScheduleJobResponse> => {
        const response = await apiInstance.put(`/schedule-jobs/${scheduleJobId}`, data)
        return response.data
    },

    // Update schedule job status
    updateScheduleJobStatus: async (scheduleJobId: string, status: 'Pending' | 'InProgress' | 'Completed'): Promise<ScheduleJobResponse> => {
